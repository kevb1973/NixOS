...
* Home-Manager
** Header
#+BEGIN_SRC nix :tangle "/home/kev/NixOS/home.nix"
{ config, pkgs, ... }:
...
exec-once = . /etc/profiles/per-user/kev/etc/profile.d/hm-session-vars.sh
...
exec-once = [workspace 8 silent] kitty http-server -p 8085 -c-1 ~/dotfiles/firefox_ext_confs/surfingkeys
...
#+BEGIN_SRC conf :tangle "/home/kev/dotfiles/hyprland/.config/hypr/hyprland.conf"
  ...
  bind = ,F9, exec, ydotool mousemove -a -x 800 -y 100 && ydotool mousemove -a -x 800 -y 90
  bind = SUPER SHIFT, Z, exec, pkill rofi || rofi -show run -a -i -n -p "Scripts:" -terminal "kitty"
...
#  bind = SUPER, code:90, exec, pgrep -i discord && hyprctl dispatch togglespecialworkspace discord || discord
  bind = SUPER, Z, exec, pkill rofi || rofi -show drun -a -i -n -I -p "Apps:" -terminal "kitty"
  bind = SUPER, b, exec, pkill waybar || waybar
  bind = SUPER, c, exec, ~/bin/show_cliphist
  bind = SUPER, code:91, exec, toggle_ncspot
  bind = SUPER SHIFT, code:91, exec, toggle_ncmpcpp
...
unmapAllExcept(["f", "q", ";e", "t", "m", "x", "<Alt-`>", "'", "T"], /www.sudokuslam.com/i);
// Unbind defaults
unmap('F');
unmap('g0');
unmap('g$');
...
addSearchAlias('t', 'Translate', 'https://translate.google.com/?hl=en&sl=auto&tl=en&text=%s&op=translate');
#+END_SRC
** Theme
